...
endings_list = {
    "secret": "Секретная",
    "diplomatic_good": "Дипломатическая (хорошая)",
    "diplomatic_bad": "Дипломатическая (плохая)",
    "military": "Военная",
    "capitalist": "Капиталистическая — предатель"
}

def load_endings():
    if not os.path.exists(ENDINGS_FILE):
        data = {k: False for k in endings_list}
...
def save_ending(name):
    endings = load_endings()
    if not endings.get(name, False):
        endings[name] = True
        with open(ENDINGS_FILE, "w", encoding="utf-8") as f:
            json.dump(endings, f, ensure_ascii=False, indent=4)
        print(f"\nНовая концовка: {endings_list[name]}!")
        input("\nНажмите Enter, чтобы вернуться обратно...")

def show_endings():
    os.system('cls')
    endings = load_endings()
    print("=== ОТКРЫТЫЕ КОНЦОВКИ ===")
    for key, title in endings_list.items():
        status = "✔ " if endings.get(key) else "✗ "
...
def home():
    ...
    prs("Утром Крош проснулся не от лучей утреннего солнца, что так мотивировало его на работу во благо всей их коммуны, весь остров в моменте поразило землетрясение. Свинина упала, подумал наш герой. А тем временем техника капиталистических мразей, вскапывающая коммунистическую землю, вырывая котлован для какой-то постройки.")
    prs("Кролик не мог оставить это все так как есть и поднявшись с кровати он надел свою фуражку с серпом, молотом и звездой и отправился чистить их земли от забугорских капиталистов, пришедших на эти земли только чтобы забрать их общее и присвоить себе.")
    print("")
...
def outside():
    ...
    print("")
    print("1. Надо бежать к Ёжику, вдруг с ним что-то случилось! Он может быть в опасности!")
    print("2. Первым делом лучше пойти к Лосяшу, он умный и что-нибудь придумает.")
...
&X++$$XXXX$XxxXXX$$+++X$XXxxxxX$$$XxXX$XXX$$$XXXxxX$$XX$XXXX$$XXXX$$X$&$xxxxxxXxxxXX$+++$XXxxxxxxxxXXXXXXXx+X&&$$$&&&&XX$&&&&$xX$                 
 ...
 $&XxX$$&&&$Xxx$$$$$$xxX$$XXXxxX$$xxx++++++++++++++++++xxX$$$Xx++++++xxx+++xXx+++++++++++++++++++x$            &&$XX$&&$XX&& &&$XX$$XxX$&         
  ...
  &XxX$XXX$$&&&&&&&&&$$$XXxxXXxxxx++++++XXx+++++++++++x$&$XxXXxxXxxxxxxxxxxxxxxxxxxxxxxxxxxxxX$$$$$$$$$XxxxxxxxxxxxxxxxxxxxXXXX$$Xx++X$$Xxx+++xX$$    
  ...
  &&&&$$$$&$$$$$$&$XXX$$$$$$XXxX$$$$$$$XxxxxX$XXXXxXXXXXXXXxxxxxxxxXXXXXXXXXXx++++++++++xXXXXXXxx+xxX$&&&&&&&$xxxxxxx++xX$Xxx++++++++x$$$XXXxxxX&$
   &&$&&&&&&&&&&&&$$$$$$$$$$$$$Xxx++++xxXXXX$$XXXxxxXXxxxxxxxxxxxxxxX$$XXxxxxxxxxx++++xx$XXXX$$X+x$&&&&$$&& &&&$XxxxxxxxXXXXXXx++++x$$XXX$&&Xxxx&$
    ...
    for line in lines:
        print(line)
    print("")
    prs("Подходя к хижине Ёжика, Крош замечает, как домик его друга пытается разнести экскаватор в щепки.")
    print("")
    ...
    if choise == "1":
        ...
        print("")
        print("Куда теперь отправиться?")
        print("")
        print("1. Первым делом лучше пойти к Лосяшу, он умный и что-нибудь придумает.")
...
def ezik_loss():
    os.system('cls')
    prs("К сожалению, ты не смог помочь ёжику. Его домик снесли, а самого Ёжика забрали в плен.")
...
def home_ezik_prod():
    os.system('cls')
    print("")
    prs("Крош смог справиться с целым экскаватором, спас коллекцию Ёжика от уничтожения.")
    prs("-Крош, спасибо! Ты сохранил мне мои любимые кактусы, - сказал Ёж и поднял пару упавших горшков. - Но я думаю это не всё. Они ещё раз захотят разрушить мой дом. С этим надо бороться.")
    prs("-Ёлки-иголки, ты прав. Мы обязаны прекратить позволять капиталистам портить наши дома, кактусы и землю. Пойдём и соберём ещё наших друзей для этого!")
    print("")
...
&xx$&           &$XXxxxxxxxXxxxxxXXX$$$Xxxxxx++++xxxX$X$$$XXx$x++++xxXXX$$$&$&                      
  ...
  &$$&     &$XXxxxxxxxxxxXX$$$$$&XXxxx++++++xx$$X$XXxxxx$XX$$xxX$x++++xxxxXXxXXX$&                  
   &$&&&  &$XXxxxxxxxxxXXX$$$$&$Xxxxx+++++++xXxXX$xxxxxxX$XXX$Xxx$xx+++xxxx$XX$XX$&                 
    ...
    if "Пин" in krosh.teams:
        print("")
        prs("Небольшая команда Кроша пришла к дому Лосяша. Крош отворил дверь, дома Лосяш сидел за столом и писал или рисовал каракули, как минимум кролик так видел все формулы, написанные учёным.")
        prs("- Лосяш, вставай, ты нам нужен. С твоей помощью мы справимся с врагами коммунизма!")
        prs("Учёный, услышав голос кролика, поднял голову и посмотрел на вошедших к нему гостей. - Друзья, а что собственно происходит?- спросил недоумённо Лосяш.")
        prs("-О майн год, к нам на остров десантировались жадные до ресурсов капиталисты. Их нужно искоренить! ")
        prs("-Господа, а какой у вас план?- сказал учёный и поднялся из-за стола.")
        prs("-Сделать оружие, с которым мы сможем уничтожить врагов коммунизма.")
        prs("-Это же никуда не годится! Можно же решить проблему дипломатически. Я за этот вариант, - произнёс Лосяш и посмотрел на Кроша.")
        prs("-Друг мой, вам какой вариант больше нравится?")
        print("")
        print("1. Пин прав, нельзя с ними договориться, проблему можно решить только создав оружие.")
        print("2. Прав Лосяш, мы должны попробовать с ними договориться.")
        krosh.revpoint += 1
        ...
        print("")
        prs("Идя дальше, Крош видел всё больше новых лиц. Это были Оптимизаторы - работники компании, они были с оружием. Крош понял сразу - лучше им на глаза не попадаться. Вскоре он был уже у дома Лосяша. Было отчётливо слышно, как роботы пилили лес, медлить нельзя, иначе их Ромашковую долину быстро превратят в руины, где когда-то жили люди.")
        prs("Войдя к Лосяшу, Крош замечает, как тот мирно сидит за столом, словно ничего и не происходило, и рисует каракули, как минимум для кролика это был непонятный набор символов. ")
        prs("")
        prs("-Лосяш, ёлки-иголки! Они вернулись, люди с большой земли! Нам нужно спасать остров от капиталистов, - произнёс кролик, пройдя к лосю, сидящему за столом.")
        prs("Учёный поднимает голову и только тогда понимает, что на улице творится неладное. - Это феноменально! Как я не заметил, слишком увлёкся исследованием. Крош, подскажи, что происходит на улице?")
        prs("-Капиталисты, они хотят забрать нашу землю. Мы не можем оставить это так!")
        prs("Отодвинув бумаги, Лосяш встал и пошёл вместе с Крошем.")

        krosh.revpoint += 1
        krosh.teams.append("Лосяш")

        print("")
        prs("Выйдя из дома, Крош около деревьев видит испуганного Бараша, он сидит на земле и размышляет о своём.")
        print("")
...
def home_svin():
    ...
    prs("Дойдя до двери, кролик, отворив её, видит, как Нюша ест очередное ведро конфет.")
    prs("- Нюша, пойдём. Капиталисты вернулись, они повсюду, они на деревьях!")
    prs("Убрав конфеты, Нюша пошла с Крошем. Она была готова помочь своим друзьям. Она вышла из своего дома.")
    print("")
...
def konfet():
    os.system('cls')
    prs("Ты поддался соблазну и всё-таки забрал Нюшины конфеты!")
...
def home_pin():
    ...
    for line in lines:
        print(line)
    print("")
    prs("Подходя к дому Пина, Крош замечает несколько роботов, что окружили его дом. Нужно помочь Пину отбиться.")
...
def home_pin_prod():
    ...
    if "Лосяш" in krosh.teams:
        print("")
        prs("Разобравшись с противниками, Крош и его небольшая команда наконец могут подняться к Пину. Взойдя на платформу под дверью, что была лифтом, они поднялись и постучали в дверь.")
        prs("-О майн год, это вы, - говорил Пин, шире открывая дверь. Он пропустил друзей к себе.")
        prs("-Что вы тут делаете в такое время? Я уже час тут сижу, а под домом эти железяки!")
        prs("-Мы поэтому к тебе и пришли. К нам десантировались капиталисты, они почти повсюду. Лосяш подтвердит.")
        prs("-Всё так. Дорогой мой друг, это чистая правда. Их слишком много, и они портят нашу экосистему, - подтвердил Лосяш.")
        prs("-И что будем делать? - спросил Пин.")
        prs("-Я предлагаю всё решить мирным путём, дипломатией, - подтвердил учёный.")
        prs("-Найн, как по мне, лучше всего сделать оружие против этого врага. Тогда мы сможем избавиться от них навсегда!")
        ...
        print("")
        prs("Разобравшись с противниками, Крош наконец может подняться к Пину. Взойдя на платформу под дверью, что была лифтом, он поднялся и постучал в дверь.")
        prs("-О майн год, я думал, железяки поняли, как пользоваться лифтом, - сказав это, Крош прошёл внутрь дома.")
        prs("-Что происходит? Я тут уже час сижу и выйти не могу. ")
        prs("-Я поэтому к тебе и пришёл. К нам десантировались капиталисты, они почти повсюду. Мне нужна твоя помощь.")
        prs("-Похоже, придётся выкинуть этих капиталистов с нашего острова.")
        print("")
        prs("Спустившись на улицу, Крош заметил, как около гаража Пина прячется Бараш. Увидев, что роботы ушли, он стал понемногу выходить из укрытия.")
        print("")
        print("Как вы поступите?")
        print("")
        print("1. Лучше взять Бараша с собой, вместе мы сильнее.")
...
def zd_pin():
    os.system('cls')
    print(" ")
    prs("Пин отправил Кроша в лес, чтобы он сразился там с несколькими роботами и раздобыл с них деталей для нового изобретения Пина.")
    print(" ")
...
def end_home():
    ...
    os.system('cls')
    print("")
    prs("Крош остался дома. Он не стал выходить на улицу и смотреть, что там происходит. Он снял фуражку и лёг на постель. Пока он спал, его друзей захватили, а их остров приватизировали себе.\n\nПоздравляем, вы открыли секретную концовку!")
...
def end_los():
    os.system('cls')
    if krosh.revpoint == 5:
        save_ending("diplomatic_good")
        print("")
        prs("Крош с друзьями собрались у корабля, который стоял в порту. На нём был главнокомандующий всей операцией по захвату острова. Обойдя охрану, они смогли пройти к нему, к тому, кто всё это начал. Лосяш как дипломат начал переговоры с главнокомандующим. По итогу они смогли договориться на хорошие условия. Смешарики остаются на острове и спокойно на нём живут, а люди с острова будут понемногу добывать ресурсы здешних земель. Крош не мог сказать, что рад результату, но это лучше, чем ничего.\n\nПоздравляем, вы вышли на дипломатическую-нейтральную концовку!")
        end = input("Нажмите [ENTER] для продолжения")
        time.sleep(3)
    if krosh.revpoint < 5:
        save_ending("diplomatic_bad")
        print("")
        prs("Крош с друзьями собрались у корабля, который стоял в порту. На нём был главнокомандующий всей операцией по захвату острова. Обойдя охрану, они смогли пройти к нему, к тому, кто всё это начал. Лосяш как дипломат начал переговоры с главнокомандующим. По итогу переговоров они не смогли договориться и всех захватили. Теперь они были одними из рабочих в городе, а их остров превратили в самое обычное место, откуда жадно качают ресурсы. Крош сильно расстроился.\n\nПоздравляем, вы вышли на дипломатическую-плохую концовку!")
        end = input("Нажмите [ENTER] для продолжения")
        time.sleep(3)
    if krosh.kappoint == 3:
        save_ending("capitalist")
        print("")
        prs("Крош с друзьями собрались у корабля, который стоял в порту. На нём был главнокомандующий всей операцией по захвату острова. Крош один прошёл к главнокомандующему и сдал своих друзей капиталистам. Он решил поменять сторону, Крош понял, что коммунизм, который устоялся у них на острове, ему не нравится и стоит принять капитализм. Так Крош стал одним из офицеров операции по захвату острова и помог забрать остальных, кто прятался. В городе он получил квартиру за свои заслуги, служа на морском флоте.\n\nПоздравляем, вы вышли на капиталистическую концовку!")
        end = input("Нажмите [ENTER] для продолжения")
        time.sleep(3)
...
def final_boss():
    os.system('cls')
    prs("Крош с командой пришли к кораблю капиталистов и вступили с ними в финальную битву.\n\nЗдоровье возросло до 200, урон до 40.")
...
def end_pin():
    ...
    if krosh.kappoint == 3:
        ...
        save_ending("military")
        print("")
        prs("Крош с друзьями собрались у корабля, который стоял в порту. На нём был главнокомандующий всей операцией по захвату острова. Взяв оружие, что им изготовил Пин, они уничтожили капиталистов, что были на корабле, а тех, что уже высадились, группа вскоре нашла и точно так же искоренила с Ромашковой долины. Теперь к ним никто не сунется, они могут за себя постоять.\n\nПоздравляем, вы вышли на военную концовку! ")
        end = input("Нажмите [ENTER] для продолжения")
        time.sleep(3)
...
def main_menu():
    ...
    if zapros == "1":
        ...
        prs("")
        prs("Comradiki (рус. Комрадики) — это классический текстовый квест с разветвлённым сюжетом на несколько концовок и увлекательной боёвкой.")
        prs("От каждого вашего (даже самого незначительного!) выбора зависит итоговая концовка, поэтому будьте внимательны к принимаемым вами решениям и их потенциальным последствиям.")
